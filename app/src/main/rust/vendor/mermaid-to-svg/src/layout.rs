...
// Android's CJK fallback fonts occupy the full 16px em. `unicode-width` reports
// those glyphs as two display units, so the normal 8px unit is the exact fit.
// The previous 6.7px estimate made Chinese state/edge labels overflow their boxes.
const STATE_CHAR_WIDTH: f64 = 8.0;
...
impl<'a> LayoutEngine<'a> {
    ...
    fn compute_with_dagre(&mut self, center_subgraph_nodes: bool) -> LayoutResult {
        ...
        let mut layout_edges: Vec<LayoutEdge> = self
            .edges
            .iter()
            .enumerate()
            .filter_map(|(idx, e)| {
                ...
                let is_self_edge = e.from == e.to;
                let is_back_edge = !is_self_edge && self.is_back_edge(&from_node, &to_node);
                let is_vertical = matches!(
                    self.graph.direction,
                    GraphDirection::TopToBottom | GraphDirection::BottomToTop
                );
                ...
                let self_edge = if is_self_edge {
                    Some(
                        self.compute_self_edge(
                            &from_node,
                            is_vertical,
                            &layout_nodes,
                            e.label
                                .as_deref()
                                .and_then(|label| self.edge_label_dimensions(label)),
                        ),
                    )
                } else {
                    None
                };
                let mut points = if let Some((points, _)) = &self_edge {
                    points.clone()
                ...
                // Self-loop endpoints are already placed on two distinct points of the node
                // boundary. Generic clipping sees the same source/target and collapses the loop.
                if !is_self_edge {
                ...
                let label_pos = e.label.as_ref().and_then(|label| {
                    if label.trim().is_empty() {
                        None
                    } else if let Some((_, label_pos)) = &self_edge {
                        Some(*label_pos)
                    } else if is_back_edge {
            ...
            .collect();
        // Dagre may occasionally place an edge label directly on a state node (for
        // example the `部署失败` label on the `回滚` target). Keep valid positions
        // unchanged, but move colliding labels to the closest clear point beside
        // their own routed edge before calculating the final viewBox.
        self.avoid_edge_label_node_overlaps(&mut layout_edges, &layout_nodes);
        let mut min_x = f64::INFINITY;
        let mut min_y = f64::INFINITY;
    ...
    fn avoid_edge_label_node_overlaps(
        &self,
        edges: &mut [LayoutEdge],
        nodes: &HashMap<String, LayoutNode>,
    ) {
        const NODE_GAP: f64 = 8.0;
        const SAMPLE_FRACTIONS: [f64; 9] = [0.5, 0.4, 0.6, 0.3, 0.7, 0.2, 0.8, 0.1, 0.9];
        const EXTRA_OFFSETS: [f64; 5] = [0.0, 16.0, 32.0, 56.0, 88.0];

        for edge in edges {
            // Self-transition geometry already deliberately places its label outside
            // the node. Leaving it alone protects the compact state-loop layout.
            ...
            let Some(label_size) = self.edge_label_dimensions(label) else {
                continue;
            };
            let original = edge
                .label_pos
                .unwrap_or_else(|| Self::edge_label_midpoint(&edge.points));
            if !Self::label_overlaps_any_node(original, label_size, nodes, NODE_GAP) {
                continue;
            }
            ...
            for fraction in SAMPLE_FRACTIONS {
                let (point, tangent) =
                    Self::edge_point_and_tangent_at_fraction(&edge.points, fraction);
                let normal = (-tangent.1, tangent.0);
                let label_normal_extent =
                    normal.0.abs() * label_size.0 / 2.0 + normal.1.abs() * label_size.1 / 2.0;

                for extra in EXTRA_OFFSETS {
                    let distance = if extra == 0.0 {
                        0.0
                    } else {
                        label_normal_extent + NODE_GAP + extra
                    };
                    let signs: &[f64] = if distance == 0.0 {
                        &[0.0]
                    } else {
                        &[-1.0, 1.0]
                    };
                    for &sign in signs {
                        let candidate = (
                            point.0 + normal.0 * distance * sign,
                            point.1 + normal.1 * distance * sign,
                        );
                        if Self::label_overlaps_any_node(candidate, label_size, nodes, NODE_GAP) {
                            continue;
                        }

                        let dx = candidate.0 - original.0;
                        let dy = candidate.1 - original.1;
            ...
            if let Some((position, _)) = best {
                edge.label_pos = Some(position);
            }
        }
    }

    fn label_overlaps_any_node(
        position: (f64, f64),
        label_size: (f64, f64),
        nodes: &HashMap<String, LayoutNode>,
        gap: f64,
    ) -> bool {
        let label_left = position.0 - label_size.0 / 2.0 - gap;
        let label_right = position.0 + label_size.0 / 2.0 + gap;
        let label_top = position.1 - label_size.1 / 2.0 - gap;
        let label_bottom = position.1 + label_size.1 / 2.0 + gap;
        ...
        nodes.values().any(|node| {
            ...
            label_right > node_left
                && node_right > label_left
                && label_bottom > node_top
                && node_bottom > label_top
        })
    }

    fn edge_point_and_tangent_at_fraction(
        points: &[(f64, f64)],
        fraction: f64,
    ) -> ((f64, f64), (f64, f64)) {
        if points.len() < 2 {
        ...
        let target_distance = total_length * fraction.clamp(0.0, 1.0);
        ...
        for (index, &segment_length) in segment_lengths.iter().enumerate() {
            ...
            if accumulated + segment_length >= target_distance {
                ...
                return (
                    (points[index].0 + dx * t, points[index].1 + dy * t),
        ...
        let dx = points[last].0 - points[last - 1].0;
        let dy = points[last].1 - points[last - 1].1;
        let length = (dx * dx + dy * dy).sqrt();
        let tangent = if length < 0.001 {
            (1.0, 0.0)
        } else {
            (dx / length, dy / length)
        };
    ...
    /// Builds a visible self-transition loop and keeps its label outside the state node.
    /// The route distance includes half of the measured label box so the opaque edge-label
    /// background cannot cover the state even for long CJK text.
    fn compute_self_edge(
        &self,
        node: &LayoutNode,
        is_vertical: bool,
        all_nodes: &HashMap<String, LayoutNode>,
        ...
        const LABEL_NODE_GAP: f64 = 12.0;
        let (label_width, label_height) = label_size.unwrap_or((0.0, 0.0));
        ...
        if is_vertical {
            ...
            let route_left = all_nodes.len() > 1 && node.x <= graph_center_x;
            let direction = if route_left { -1.0 } else { 1.0 };
            let boundary_x = node.x + direction * node.width / 2.0;
            let route_x = boundary_x + direction * (label_width / 2.0 + LABEL_NODE_GAP);
            let anchor_offset = (node.height * 0.22).max(7.0);
            let start = (boundary_x, node.y - anchor_offset);
            let end = (boundary_x, node.y + anchor_offset);
            (
                vec![start, (route_x, start.1), (route_x, end.1), end],
                (route_x, node.y),
            )
        } else {
            let min_y = all_nodes
            ...
            let route_above = all_nodes.len() > 1 && node.y <= graph_center_y;
            let direction = if route_above { -1.0 } else { 1.0 };
            let boundary_y = node.y + direction * node.height / 2.0;
            let route_y = boundary_y + direction * (label_height / 2.0 + LABEL_NODE_GAP);
            let anchor_offset = (node.width * 0.22).max(7.0);
            let start = (node.x - anchor_offset, boundary_y);
            let end = (node.x + anchor_offset, boundary_y);
            (
                vec![start, (start.0, route_y), (end.0, route_y), end],
                (node.x, route_y),
            )
        }
    }
...
mod tests {
    ...
    #[test]
    fn state_self_transitions_are_visible_and_labels_stay_outside_nodes() {
        let graph = crate::state_diagram::parse_state_diagram(
            "stateDiagram-v2\n[*] --> 静止\n静止 --> 向上: 按上行按钮\n静止 --> 向下: 按下行按钮\n向上 --> 向上: 经过楼层（不停）\n向下 --> 向下: 经过楼层（不停）",
        )
        .expect("state diagram parses");
        let layout = compute_layout(&graph);
        let self_edges: Vec<&LayoutEdge> = layout
            .edges
            .iter()
        ...
        for edge in self_edges {
            let node = layout.nodes.get(&edge.from).expect("self-edge node");
            let label_pos = edge.label_pos.expect("self-edge label position");
            assert!(edge.points.len() >= 4, "self loop must have a routed path");
            assert!(
                label_pos.0 < node.x - node.width / 2.0 || label_pos.0 > node.x + node.width / 2.0,
                "label {label_pos:?} must stay outside node {node:?}",
            );
        }

        assert!(
            layout.nodes.get("静止").expect("CJK node").width >= 38.0,
            "two CJK glyphs need their full 16px advances plus node padding",
        );
    }

    #[test]
    fn state_edge_labels_do_not_cover_nodes() {
        let graph = crate::state_diagram::parse_state_diagram(
            "stateDiagram-v2\n[*] --> 开发中\n开发中 --> 提测: 提交测试\n提测 --> 测试中: QA开始\n测试中 --> 提测: 发现Bug\n测试中 --> 待审批: 测试通过\n待审批 --> 审批中: 管理员开始审批\n审批中 --> 待审批: 审批未通过\n审批中 --> 已批准: 审批通过\n已批准 --> 部署中: 自动触发\n部署中 --> 已上线: 部署成功\n部署中 --> 回滚: 部署失败\n回滚 --> 开发中: 重新修复\n已上线 --> [*]",
        )
        .expect("state diagram parses");
        ...
        for edge in &layout.edges {
            ...
            let Some((label_width, label_height)) = engine.edge_label_dimensions(label) else {
                continue;
            };
            let (label_x, label_y) = edge.label_pos.expect("labeled edge position");
            for node in layout.nodes.values() {
                let overlap_x = label_x + label_width / 2.0 > node.x - node.width / 2.0
                    && node.x + node.width / 2.0 > label_x - label_width / 2.0;
                let overlap_y = label_y + label_height / 2.0 > node.y - node.height / 2.0
                    && node.y + node.height / 2.0 > label_y - label_height / 2.0;
                assert!(
                    !(overlap_x && overlap_y),
                    "edge label {label:?} at ({label_x:.1}, {label_y:.1}) overlaps node {:?} at ({:.1}, {:.1})",
                    node.id,
                    node.x,
                    node.y,
                );
