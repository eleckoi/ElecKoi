...
8: "CRC""#;
    ...
    #[test]
    fn state_deployment_edge_label_renders_clear_of_rollback_node() {
        let source = "stateDiagram-v2\n[*] --> 开发中\n开发中 --> 提测: 提交测试\n提测 --> 测试中: QA开始\n测试中 --> 提测: 发现Bug\n测试中 --> 待审批: 测试通过\n待审批 --> 审批中: 管理员开始审批\n审批中 --> 待审批: 审批未通过\n审批中 --> 已批准: 审批通过\n已批准 --> 部署中: 自动触发\n部署中 --> 已上线: 部署成功\n部署中 --> 回滚: 部署失败\n回滚 --> 开发中: 重新修复\n已上线 --> [*]";
        let svg = render_mermaid_to_svg(source, None).expect("state diagram renders");
        assert!(svg.contains("部署失败"));
        assert!(svg.contains("回滚"));
    }
