...
// Keep renderer label boxes identical to the state layout's CJK-aware metrics.
...
impl<'a> SvgRenderer<'a> {
    ...
    fn render(&mut self, layout: &LayoutResult) -> String {
        ...
        self.render_edge_labels(&layout.edges, &layout.nodes);
...
"#,
    ...
    fn render_edge_labels(
        &mut self,
        edges: &[LayoutEdge],
        nodes: &std::collections::HashMap<String, LayoutNode>,
    ) {
        struct LabelInfo {
            x: f64,
            y: f64,
            ...
            for i in 0..labels.len() {
                for j in (i + 1)..labels.len() {
                    ...
                    if overlap_x && overlap_y {
                        any_collision = true;
                        let original_i = (labels[i].x, labels[i].y);
                        let original_j = (labels[j].x, labels[j].y);
                        ...
                        // Layout has already placed every label clear of state nodes. Pairwise
                        // label separation must not undo that guarantee (the old renderer moved
                        // `部署失败` back over the `回滚` state here). Roll back only the unsafe
                        // half; later iterations can move the other label instead.
                        if Self::label_box_overlaps_nodes(
                            labels[i].x,
                            labels[i].y,
                            labels[i].width,
                            labels[i].height,
                            ...
                            (labels[i].x, labels[i].y) = original_i;
                        }
                        if Self::label_box_overlaps_nodes(
                            labels[j].x,
                            labels[j].y,
                            ...
                            (labels[j].x, labels[j].y) = original_j;
                        }
                    }
                }
            }
        ...
        for info in &mut labels {
            // Belt-and-suspenders fallback for future collision-resolution changes: the layout
            // position is node-safe, so never render a later adjusted label on top of a node.
            if Self::label_box_overlaps_nodes(
                info.x,
                info.y,
                info.width,
                ...
                info.x = info.layout_x;
                info.y = info.layout_y;
            }
            debug_assert!(
                !Self::label_box_overlaps_nodes(
                    info.x,
                    info.y,
                    info.width,
                    info.height,
                ...
                "edge label must not render on top of a node",
            );
...
"#,
    ...
    fn label_box_overlaps_nodes(
        ...
        let left = x - width / 2.0 - gap;
        let right = x + width / 2.0 + gap;
        let top = y - height / 2.0 - gap;
        let bottom = y + height / 2.0 + gap;
        nodes.values().any(|node| {
            ...
            right > node_left && node_right > left && bottom > node_top && node_bottom > top
        })
    }
